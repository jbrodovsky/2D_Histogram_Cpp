...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	// your code here
	float p = 1.0 / (grid.size() * grid[0].size());
  	vector< vector<float> > newGrid(grid.size(), vector<float>(grid[0].size(), p));
	return newGrid;
}
...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring){
	...
	for(int i = 0; i<beliefs.size(); i++){
      for(int j = 0; j<beliefs[0].size(); j++){
        int new_i = fmod(i + dx, beliefs.size());
        int new_j = fmod(j + dy, beliefs[0].size());
        //if(new_i>=beliefs.size()){ new_i = new_i - beliefs.size(); }
        //if(new_j>=beliefs[0].size()){ new_j = new_j - beliefs[0].size(); }
