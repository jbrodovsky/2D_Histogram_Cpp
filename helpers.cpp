...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    //vector<vector<float> > newGrid (grid.size(), vector<float>(0, grid[0].size));
    for(int i = 0; i<grid.size(); i++){
      for(int j = 0; j<grid[0].size(); j++){
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	//std::cout<<"We're now in blur()"<<std::endl;
	//vector < vector <float> > newGrid;
	// your code here
  	int h = grid.size();
  	int w = grid[0].size();
	...
	vector < vector <float> > newGrid(h, vector<float> (w, 0) );
  	
  	for(int i = 0; i<h; i++){
      for(int j = 0; j<w; j++){
        float val = grid[i][j];
        //cout<<"Blurring grid value "<<val<<" at ("<<i<<","<<j<<")"<<endl;
        for(int dx = -1; dx<2; dx++){
          for(int dy = -1; dy<2; dy++){
            float mult = window[dx+1][dy+1];
            int new_i = (i+dx+h)%h;	//fmod(i+dy, h);
            int new_j = (j+dy+w)%w;	//fmod(j+dx, w);
            //if(new_i>h){new_i = new_i - h;}
            //if(new_j>w){new_j = new_j - w;}
            //cout<<"New I: "<<new_i<<" | New J: "<<new_j<<" = "<<mult*val<<endl;
            newGrid[new_i][new_j] += mult * val;
          }
        }
      }
    }
